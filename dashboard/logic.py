...
class DashboardLogic:
    ...
    # ── Analytics ─────────────────────────────────────────────────────────────
    def daily_summary(self, profile: Optional[str] = None) -> List[Dict[str, Any]]:
        slips = self.get_slips(profile or "all")
        ...
        for day in sorted_days:
            ...
            summary.append({
                ...
                "win_rate": round((cum_won_count / cum_settled_count * 100) if cum_settled_count else 0, 2)
            })

        return summary

    def market_accuracy(self, profile: Optional[str] = None) -> List[Dict[str, Any]]:
        slips = self.get_slips(profile or "all")
        ...
        for slip in slips:
            for leg in slip["legs"]:
                ...
                if mtype not in market_stats:
                    market_stats[mtype] = {"market": mtype, "won": 0, "lost": 0, "total": 0}
    ...
    def correlation_data(self, profile: Optional[str] = None) -> List[Dict[str, Any]]:
        slips = self.get_slips(profile or "all")
        settled = [s for s in slips if s["slip_status"] in ("Won", "Lost")]
        ...
        for s in settled:
            data.append({
                ...
                "profit": round((s["total_odds"] * s["units"] - s["units"]) if s["slip_status"] == "Won" else -s["units"], 2)
            })
        return data
