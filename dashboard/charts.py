...
def render_history_charts(daily_summary: List[dict]):
    ...
    fig_bet.add_trace(go.Scatter(x=dates, y=[d["cumulative_bet"] for d in daily_summary],
                                 name="Total Bet", line=dict(color=CHART_COLORS[0], width=3),
                                 fill='tozeroy', fillcolor='rgba(0, 123, 255, 0.1)'))
    fig_bet.update_layout(title="Cumulative Units Bet", xaxis_title="Date", yaxis_title="Units")

    # 2. Total Profit over time
    fig_profit = go.Figure(layout=CHART_LAYOUT)
    fig_profit.add_trace(go.Scatter(x=dates, y=[d["cumulative_profit"] for d in daily_summary],
                                    name="Profit", line=dict(color=COLORS["success"] if daily_summary[-1]["cumulative_profit"] >= 0 else COLORS["danger"], width=3),
                                    fill='tozeroy', fillcolor='rgba(40, 167, 69, 0.1)'))
    fig_profit.update_layout(title="Cumulative Net Profit", xaxis_title="Date", yaxis_title="Units")

    # 3. ROI over time
    fig_roi = go.Figure(layout=CHART_LAYOUT)
    fig_roi.add_trace(go.Scatter(x=dates, y=[d["roi_percentage"] for d in daily_summary],
                                 name="ROI %", line=dict(color=CHART_COLORS[2], width=2)))
    fig_roi.update_layout(title="ROI % Over Time", xaxis_title="Date", yaxis_title="ROI %")

    # 4. Win Rate over time
    fig_winrate = go.Figure(layout=CHART_LAYOUT)
    fig_winrate.add_trace(go.Scatter(x=dates, y=[d["win_rate"] for d in daily_summary],
                                     name="Win Rate %", line=dict(color=CHART_COLORS[3], width=2)))
    fig_winrate.update_layout(title="Win Rate % Over Time", xaxis_title="Date", yaxis_title="Win Rate %")
...
def render_market_accuracy_chart(market_stats: List[dict]):
    ...
    fig.update_layout(barmode='group', title="Market Accuracy (Won vs Lost)",
                      xaxis_title="Market Type", yaxis_title="Count")
    return _graph(fig)
...
def render_correlation_charts(correlation_data: List[dict]):
    ...
    legs_stats = df.groupby("legs_count").agg(
        total=("status", "count"),
    ...
    fig_legs.add_trace(go.Bar(x=legs_stats["legs_count"], y=legs_stats["win_rate"],
                              marker_color=CHART_COLORS[4]))
    fig_legs.update_layout(title="Win Rate by Number of Legs",
                           xaxis_title="Number of Legs", yaxis_title="Win Rate %")

    # 2. Profit Distribution by Legs Count
    fig_profit_legs = go.Figure(layout=CHART_LAYOUT)
    for legs in sorted(df["legs_count"].unique()):
        fig_profit_legs.add_trace(go.Box(y=df[df["legs_count"] == legs]["profit"],
                                         name=f"{legs} Legs"))
    fig_profit_legs.update_layout(title="Profit Distribution by Legs Count",
                                  xaxis_title="Number of Legs", yaxis_title="Profit (Units)")
