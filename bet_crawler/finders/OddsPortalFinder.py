...
class OddsPortalFinder(BaseMatchFinder):
        ...
        with browser(solve_cloudflare=True, interactive=True, disable_resources=False, headless=True) as session:
            for url in urls:
                try:
                    ...
                    match_date = datetime.strptime(soup.select_one('[data-testid="game-time-item"] p:nth-of-type(2)').text.strip().rstrip(','), "%d %B %Y").replace(hour=0, minute=0, second=0)
                    ...
                    try:
                        ...
                        soup = BeautifulSoup(session.page.content(), "html.parser")
                        cells = soup.find('div', {'data-testid': 'over-under-expanded-row'}).find_all('div', {'data-testid': 'odd-container'})
                    ...
                    odds = Odds(
                        home=odds_1, draw=odds_X, away=odds_2,
                        over_05=odds_over05, under_05=odds_under05,
                        over_15=odds_over15, under_15=odds_under15,
                        over_25=odds_over25, under_25=odds_under25,
                        over_35=odds_over35, under_35=odds_under35,
                        over_45=odds_over45, under_45=odds_under45,
                        btts_y=odds_btts_y, btts_n=odds_btts_n,
                        dc_1x=odds_dc_1x, dc_12=odds_dc_12, dc_x2=odds_dc_x2,
                    )

                    logger.info("[%s] %s", thread_name, odds)

                    with self._add_match_lock:
                        self.add_match(Match(home_team=home_team, away_team=away_team, datetime=match_date, predictions=None, odds=odds))
    ...
    def get_matches(self, urls) -> None:
        ...
        logger.info(
            ...
            len(urls), len(chunks), [len(c) for c in chunks],
        )
