"""
BetSlipManager - Database Manager for Betting Parlays

This module provides a database manager for handling betting parlay slips and their individual legs.
It manages the storage, retrieval, and status tracking of betting slips generated by the BettingAnalyzer.

The system maintains two main tables:
- 'slips': Stores information about each parlay slip including date, profile, total odds, and units
- 'legs': Stores individual bets within each slip, including match details, market type, odds, and status

Features include:
- Inserting new betting slips with multiple legs
- Tracking leg statuses (Pending, Won, Lost)
- Calculating betting statistics (ROI, win rate, profit/loss)
- Managing database connections with automatic reopening when file changes
- Filtering by profile for targeted analytics
"""
import sqlite3
import math
import os
